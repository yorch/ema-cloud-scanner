...
class BacktestResult:
    ...
    def calculate_metrics(self):
        ...
        self.expectancy = (win_rate_decimal * self.avg_win) - ((1 - win_rate_decimal) * self.avg_loss)
...
class Backtester:
    ...
    def run(
        ...
        for i in range(50, len(df)):
            ...
            if position is None and current_signal is not None:
                ...
                if isinstance(sig_strength, str):
                    strength_map = {'VERY_STRONG': 5, 'STRONG': 4, 'MODERATE': 3, 'WEAK': 2, 'VERY_WEAK': 1}
        ...
        max_dd_pct = (max_dd / rolling_max[max_dd_idx]) * 100 if max_dd_idx and rolling_max[max_dd_idx] > 0 else 0
    ...
    def compare_results(self, results: Dict[str, BacktestResult]) -> pd.DataFrame:
        ...
        for symbol, result in results.items():
            rows.append({
                ...
                'Profit Factor': result.profit_factor if result.profit_factor != float('inf') else 999,
