...
class CustomHoldingsProvider(BaseHoldingsProvider):
    ...
    async def get_holdings(self, etf_symbol: str) -> ETFHoldings | None:
        ...
        holdings = [
            Holding(symbol=h["symbol"], name=h.get("name", h["symbol"]), weight=h.get("weight", 0))
...
class HoldingsManager:
    ...
    def __init__(self, cache_duration_hours: int = 24, custom_config_path: str | None = None):
    ...
    async def get_holdings(
        self, etf_symbol: str, force_refresh: bool = False, use_custom: bool = True
    ) -> ETFHoldings:
        """
    ...
    async def get_all_holdings(
        self, etf_symbols: list[str], top_n: int = 10
    ...
    def get_all_stock_symbols(self, etf_symbols: list[str], top_n: int = 10) -> list[str]:
        """
