...
from .config.settings import (
    ScannerConfig, TradingStyle, SECTOR_ETFS, ETF_SUBSETS,
    TRADING_PRESETS, FilterConfig
)
from .data_providers.base import DataProviderManager, YahooFinanceProvider
from .indicators.ema_cloud import EMACloudIndicator, TechnicalIndicators
from .signals.generator import SignalGenerator, Signal, SectorTrendState
from .holdings.manager import HoldingsManager
from .alerts.handlers import AlertManager, AlertMessage, create_alert_from_signal
from .visualization.dashboard import (
    TerminalDashboard, SimpleDashboard, ETFDisplayData, SignalDisplayData
)
...
class MarketHours:
    ...
    @classmethod
    def is_extended_hours(cls, check_time: Optional[datetime] = None) -> bool:
    ...
    @classmethod
    def time_to_open(cls, check_time: Optional[datetime] = None) -> Optional[timedelta]:
...
class EMACloudScanner:
    ...
    def __init__(self, config: Optional[ScannerConfig] = None):
        ...
        self.data_manager.add_provider(YahooFinanceProvider())
        
        self.cloud_indicator = EMACloudIndicator(
            ema_configs=self.config.ema_configs
        )
    ...
    def _get_etf_list(self) -> List[str]:
        ...
        if self.config.etf_symbols:
            return self.config.etf_symbols
        
        if self.config.etf_subset in ETF_SUBSETS:
            return ETF_SUBSETS[self.config.etf_subset]
    ...
    async def analyze_etf(self, symbol: str) -> Optional[Dict]:
        """
    ...
    async def scan_all_etfs(self) -> List[Dict]:
        ...
        for etf, analysis in zip(etfs, analyses):
    ...
    async def process_signals(self, analyses: List[Dict]):
        ...
        for analysis in analyses:
            for signal in analysis.get('signals', []):
                if signal.is_valid() and self._should_alert_signal(signal):
                    # Create alert
                    ...
                    # Update dashboard
                    if self.dashboard:
                        self.dashboard.add_signal(SignalDisplayData(
    ...
    def _update_dashboard(self, analyses: List[Dict]):
        ...
        for analysis in analyses:
            ...
            self.dashboard.update_etf_data(ETFDisplayData(
                ...
                cloud_state=trend.primary_cloud_state.name if trend.primary_cloud_state else "UNKNOWN",
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        help='Specific ETFs to scan (e.g., XLK XLF XLV)'
    )
    
    parser.add_argument(
        '--subset',
        type=str,
        choices=list(ETF_SUBSETS.keys()),
        help='ETF subset to scan'
    )
