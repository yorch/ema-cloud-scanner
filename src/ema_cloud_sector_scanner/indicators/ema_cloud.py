...
class EMACloudIndicator:
    ...
    def __init__(self, clouds_config: Optional[Dict[str, Tuple[int, int]]] = None):
        """
    ...
    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for name, (fast, slow) in self.clouds_config.items():
            ...
            result[f'{name}_thickness'] = result[f'{name}_cloud_top'] - result[f'{name}_cloud_bottom']
    ...
    def get_price_relation(self, price: float, cloud_top: float, cloud_bottom: float, 
                          tolerance_pct: float = 0.1) -> PriceRelation:
    ...
    def analyze_single(self, df: pd.DataFrame, idx: int = -1) -> Dict[str, CloudData]:
        ...
        for name in self.clouds_config.keys():
            fast = row[f'{name}_fast']
            slow = row[f'{name}_slow']
            ...
            if idx >= 3:
                ...
                prev_mid = (df.iloc[idx - 3][f'{name}_cloud_top'] + 
                           df.iloc[idx - 3][f'{name}_cloud_bottom']) / 2
    ...
    def detect_signals(self, df: pd.DataFrame, idx: int = -1) -> List[str]:
        ...
        if 'pullback' in clouds:
            ...
            if pb.price_relation == PriceRelation.TOUCHING_BOTTOM:
                if 'trend_confirmation' in clouds and clouds['trend_confirmation'].state == CloudState.BULLISH:
                    signals.append("🟢 PULLBACK_ENTRY: Price at 8-9 cloud support in uptrend")
            elif pb.price_relation == PriceRelation.TOUCHING_TOP:
                if 'trend_confirmation' in clouds and clouds['trend_confirmation'].state == CloudState.BEARISH:
                    signals.append("🔴 PULLBACK_ENTRY: Price at 8-9 cloud resistance in downtrend")
        
        # Multi-cloud alignment
        bullish_count = sum(1 for c in clouds.values() if c.state in [CloudState.BULLISH, CloudState.CROSSING_UP])
        bearish_count = sum(1 for c in clouds.values() if c.state in [CloudState.BEARISH, CloudState.CROSSING_DOWN])
